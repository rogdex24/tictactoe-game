...
1. Start Postgres and Nakama services using Docker Compose (recommended):
   ...
   **For production (with nginx and SSL):**
   ...
   Both options start PostgreSQL and the Nakama server with the compiled Go plugin.
...
- `docker compose -f backend/docker-compose.dev.yml ps` (or `docker-compose.yml` for production) shows both `postgres` and `nakama` containers in the `running` state.
...
- If services fail to start, check that Docker is running and try: `docker compose -f backend/docker-compose.dev.yml down && docker compose -f backend/docker-compose.dev.yml up -d` (use `docker-compose.yml` for production)
...
- Stale Docker resources can be cleaned via `docker compose -f backend/docker-compose.dev.yml down --remove-orphans`.
